...
static void adapt_energy_weight(void)
{
        ...
        if (w_energy != prev) {
                LOG_DBG("LTE-U: w_e %d->%d (tp=%d fp=%d n=%d scale=%d)",
...
static void connected_cb(struct bt_conn *conn, uint8_t err)
{
        ...
        /* VS HCI enables (conn event reporting, channel survey, power control)
         * are deferred to security_changed_cb — they should only fire after
         * the link is encrypted. Keep handle tracking above (pure bookkeeping). */
}
